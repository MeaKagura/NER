...
def train_epoch(train_loader, model, optimizer, scheduler, epoch):
    # set ner_model to training mode
    model.train()
    # step number in one epoch: 336
    train_losses = 0
    for idx, batch_samples in enumerate(tqdm(train_loader)):
        batch_sentences, batch_token_starts, batch_labels = batch_samples
        batch_masks = batch_sentences.gt(0)  # get padding mask
        # compute ner_model output and loss
        loss = model((batch_sentences, batch_token_starts),
...
def train(train_loader, dev_loader, model, optimizer, scheduler, model_dir):
    """train the ner_model and test ner_model performance"""
    # reload weights from restore_dir if specified
    if model_dir is not None and config.load_before:
        model = BertNER.from_pretrained(model_dir)
        model.to(config.device)
        logging.info("--------load ner_model from {}--------".format(model_dir))
    best_val_f1 = 0.0
    patience_counter = 0
    # start training
    for epoch in range(1, config.epoch_num + 1):
        ...
        if improve_f1 > 1e-5:
            ...
            logging.info("--------the best ner_model is saved.--------")
        ...
        for idx, batch_samples in enumerate(dev_loader):
            ...
            if mode == 'test':
                sent_data.extend([[tokenizer.convert_ids_to_tokens(idx.item()) for idx in indices
                                   if (idx.item() > 0 and idx.item() != 101)] for indices in batch_sentences])
            batch_masks = batch_sentences.gt(0)  # get padding mask, gt(x): get index greater than x
            ...
            batch_output = model((batch_sentences, batch_token_starts),
                                 token_type_ids=None, attention_mask=batch_masks)[0]
            # (batch_size, max_len - padding_label_len)
            batch_output = model.crf.decode(batch_output, mask=label_masks)
            # (batch_size, max_len)
            batch_labels = batch_labels.to('cpu').numpy()
            pred_labels.extend([[id2label.get(idx) for idx in indices] for indices in batch_output])
            # (batch_size, max_len - padding_label_len)
            true_labels.extend([[id2label.get(idx) for idx in indices if idx > -1] for indices in batch_labels])

    assert len(pred_labels) == len(true_labels)
    if mode == 'test':
        assert len(sent_data) == len(true_labels)

    # logging loss, f1 and report
    metrics = {}
    if mode == 'dev':
        f1 = f1_score(true_labels, pred_labels, mode)
        metrics['f1'] = f1
    else:
        bad_case(true_labels, pred_labels, sent_data)
        f1_labels, f1 = f1_score(true_labels, pred_labels, mode)
